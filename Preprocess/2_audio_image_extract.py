...
def scilenceRemove(INPUT_FILE, OUTPUT_FILE, frameRate):
    ...
    TEMP_FOLDER = os.path.join(os.path.splitext(OUTPUT_FILE)[0])
    print('Out put director (TEMP_FOLDER) is:', TEMP_FOLDER)
    ...
    command = "ffmpeg -i " + INPUT_FILE + " -ab 160k -ac 2 -ar " + str(
        SAMPLE_RATE) + " -vn " + TEMP_FOLDER + "/audio.wav"  # -ab: audio bitrate
    # print('audio.wav generater command: %s' % command)
    subprocess.call(command, shell=True)
    ...
    # subprocess.call(command, shell=True)  # -i: input file
...
assert INPUT_FILES != None, "why u put no input file, that dum"
print('INPUT_FILES is: %s' % INPUT_FILES)

if os.path.isdir(INPUT_FILES):
    print('will process more than 1 file')
    for root, dires, files in os.walk(os.path.join(INPUT_FILES, 'ravdess')):
        if files:  # and 'Actor_01' not in root:
            for file in files:
                if file.endswith(".mp4") and file[1] == '1':
                    print('input file: %s \n output file: %s' % (os.path.join(root, file),
                                                                 os.path.join(
                                                                     root.replace('ravdess', 'data_preprocess'), file)))
                    # if not os.path.isdir(root.replace('ravdess', 'data_preprocess')):
                    #     print('make director %s' % root.replace('ravdess', 'data_preprocess'))
                    #     os.makedirs(root.replace('ravdess', 'data_preprocess'))
                    # createPath(root.replace('ravdess', 'data_preprocess'))
                    # scilenceRemove(os.path.join(root, file),
                    #                os.path.join(root.replace('ravdess', 'data_preprocess'), file),
                    #                frameRate)

                    print('input file: %s' % os.path.join(root.replace('ravdess', 'data_preprocess'), file))
                    scilenceRemove(os.path.join(root.replace('ravdess', 'data_preprocess'), file),
                                   os.path.join(root.replace('ravdess', 'data_preprocess'), file),
                                   frameRate)
else:
    print('will process 1 file')
    scilenceRemove(INPUT_FILES, '', frameRate)
